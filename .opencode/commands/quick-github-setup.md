...
Run setup scripts to configure GitHub automation in one command.
...
# Setup OpenCode secrets
if [ "$AGENT" == "opencode" ] || [ "$AGENT" == "both" ]; then
  ...
  echo "Generating OpenCode OAuth token..."
  TOKEN=$(opencode setup-token 2>&1 | tail -1)
  echo "$TOKEN" | gh secret set OPENCODE_OAUTH_TOKEN
...
if [ "$AGENT" == "both" ]; then
  ...
  echo "✅ Using multi-agent workflow (OpenCode + Codex)"
elif [ "$AGENT" == "opencode" ]; then
  # OpenCode-only workflow is already in .github/workflows/opencode-fix-coderabbit.yml
  echo "✅ Using OpenCode workflow"
...
if [ ! -f .coderabbit.yaml ]; then
  ...
  cp templates/.coderabbit.yaml .coderabbit.yaml 2>/dev/null || echo "⚠️  .coderabbit.yaml template not found"
fi
...
echo "Usage:"
if [ "$AGENT" == "both" ]; then
  echo "  - Comment '@opencode-fix' on a PR → OpenCode fixes it"
  echo "  - Comment '@codex-fix' on a PR → Codex fixes it"
  echo "  - CodeRabbit reviews automatically use OpenCode"
else
...
- Requires: `gh`, `opencode` (for OpenCode), `codex` (for Codex)
