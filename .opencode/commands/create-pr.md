...
**If DocWriter available** (`ls .opencode/agents/subagent-docwriter.md`) AND significant changes (>5 files):
- Consider invoking `@subagent-docwriter` to generate detailed change documentation for PR body
- DocWriter adds value for large PRs — small PRs (1-5 files) don't need it

**If agent not available OR small PR**: Proceed with manual PR content drafting.
...
Before creating PR, show the user:
...
3. If yes, inform the user that CodeRabbit will review automatically
4. Optionally offer to open the PR in the browser
...
- If push fails, show the error and suggest checking permissions
