...
- Install GitHub CLI: `winget install GitHub.cli` (Windows) or `brew install gh` (macOS), then `gh auth login`
...
### 1.5. Agent-Assisted Context Gathering (Optional)

**If ContextScout available** (`ls .opencode/agents/subagent-contextscout.md`):
- Launch `@subagent-contextscout` to find error handling patterns, related code, and similar issue resolutions in the codebase
- Query: "error handling patterns for {component mentioned in issue}"

**If ExternalScout available** (`ls .opencode/agents/subagent-externalscout.md`) AND issue mentions external libraries:
- Launch `@subagent-externalscout` to fetch current documentation for the library
- Query: "{library name} error handling documentation"

**If agents not available**: Skip — proceed to manual codebase search.
...
3. Run: `/commit` after the implementation is complete
```
