...
2. Available authentication credentials:
   - OpenCode: `~/.opencode/auth.json` or output from `opencode setup-token`
   - Codex: `~/.codex/auth.json`
3. Which agent(s) to set up: opencode, codex, or both
...
OPENCODE_AUTH_EXISTS=$([ -f ~/.opencode/auth.json ] && echo "true" || echo "false")
...
Based on `$ARGUMENTS`:
- `opencode` → Set up only OpenCode
- `codex` → Set up only Codex
- `both` or empty → Set up both (if credentials available)

Check availability:
```bash
if [ "$AGENT_CHOICE" == "opencode" ] || [ "$AGENT_CHOICE" == "both" ]; then
  if [ "$OPENCODE_AUTH_EXISTS" == "false" ]; then
    echo "⚠️  OpenCode not authenticated. Run: opencode login"
...
### Step 2: Setup OpenCode Secrets

If OpenCode selected:

```bash
echo "🔐 Setting up OpenCode secrets..."

# Method 1: Extract from auth.json
if [ -f ~/.opencode/auth.json ]; then
  OPENCODE_TOKEN=$(jq -r '.oauth_token' ~/.opencode/auth.json 2>/dev/null || grep -o '"oauth_token"[[:space:]]*:[[:space:]]*"[^"]*"' ~/.opencode/auth.json | cut -d'"' -f4)
fi

# Method 2: Prompt user to run setup-token
if [ -z "$OPENCODE_TOKEN" ]; then
  echo "Running: opencode setup-token"
  OPENCODE_TOKEN=$(opencode setup-token 2>&1 | grep -o 'sk-ant-[A-Za-z0-9_-]*')
fi

# Add to GitHub
echo "$OPENCODE_TOKEN" | gh secret set OPENCODE_OAUTH_TOKEN -R "$REPO"
echo "✅ OPENCODE_OAUTH_TOKEN configured"
```
...
# Create a PR
gh pr create --title "test: verify automation setup" --body "This PR tests automated review-fix loop." --base master
...
3. Comment on PR:
   - `@opencode-fix` → OpenCode applies fixes
...
**If OpenCode auth not found:**
```bash
echo "❌ OpenCode authentication not found"
echo ""
echo "Setup OpenCode:"
echo "  1. Run: opencode login"
echo "  2. Run: opencode setup-token"
