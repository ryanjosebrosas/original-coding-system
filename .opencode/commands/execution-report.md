...
## Step 1: Load Reference Patterns (Optional)

**If ContextScout available** (`ls .opencode/agents/subagent-contextscout.md`):
- Launch `@subagent-contextscout` to discover what patterns in `reference/` and `sections/` should have been followed
- Query: "patterns for {components implemented}"
- This provides baseline for comparing what was implemented vs what should have been followed

**If agent not available**: Skip — rely on conversation context for pattern comparison.
