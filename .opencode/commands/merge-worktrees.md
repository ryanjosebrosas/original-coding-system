...
**Critical rule**: ALL validations must pass before continuing.

---

### Step 4.5: Agent-Assisted Comprehensive Validation (Optional)
...
- BuildAgent detects project type and runs lint → typecheck → build in pyramid order
- Review any additional issues found — BuildAgent catches what project scripts miss

**If agent not available**: Skip — rely on Step 4 validation.
...
**If user chooses "Yes" — Plugin Mode** (if `worktree_delete` tool available):

For EACH merged branch:
```
worktree_delete:
  reason: "Merged to {target-branch}, cleanup complete"
```

Plugin handles: auto-commit (if uncommitted changes), git worktree remove, session cleanup.

**If user chooses "Yes" — Manual Mode** (fallback):
...
To clean up manually (plugin):
  worktree_delete with reason: "Manual cleanup"

To clean up manually (git):
  git worktree remove worktrees/{branch_name}
  git branch -d {branch_name}
```

**Note**: This command works with both plugin-managed and manual worktrees.
...
- Test merges without affecting the main branch
...
- Use the question tool for user preference (not hardcoded)
- Some users want to keep worktrees for continued iteration
- Some users want clean repository state
- Both choices are valid — let the user decide
