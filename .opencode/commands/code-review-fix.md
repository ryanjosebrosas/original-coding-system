...
**b.** State what's wrong, why it's a problem. Reference to finding (severity, file:line).
...
### Step 5.5: Agent-Assisted Build Validation (Optional)
...
- BuildAgent detects project type and runs appropriate validation chain
- Reviews findings — BuildAgent is read-only (reports, doesn't fix)

**If agent not available**: Skip — rely on Step 5 validation.
