...
This command chains 4 core commands for autonomous feature development.
...
### Step 2.5: Task Complexity Analysis (Optional)

**If TaskManager available** (`ls .opencode/agents/subagent-taskmanager.md`) AND plan has >8 tasks:
- Launch `@subagent-taskmanager` to analyze task dependencies and identify parallel batch opportunities
- This is optional for simple features but recommended for complex multi-file implementations

**If agent not available OR <8 tasks**: Skip — proceed to execution.

---

## Step 3: Execute - Implement Feature
...
### Step 3.5: Agent-Assisted Validation (Optional)

**If BuildAgent available** (`ls .opencode/agents/subagent-buildagent.md`):
- Launch `@subagent-buildagent` for comprehensive build validation (lint → typecheck → build)
- Review findings before committing

**If TestEngineer available** (`ls .opencode/agents/subagent-testengineer.md`) AND tests appear incomplete:
- Launch `@subagent-testengineer` to add test coverage for edge cases
- This is high-trust — agent failures should warn but not block the workflow

**If agents not available**: Skip — rely on plan's validation commands.
...
5. **Capture the PR URL** and number from the output

**Error Handling:**
- If `gh` CLI is not available, provide manual instructions
- If already on master/main, skip PR creation and warn the user
- If PR already exists for this branch, skip and show the existing PR URL
...
**Next Steps:**
- CodeRabbit will review PR automatically (if configured)
